...
fn main() {
    ...
    // Composite: χ * ratio
    let data_chi_ratio: Vec<Vec<f64>> =
        data.iter().map(|v| vec![v[0], v[1], v[0] * v[6]]).collect();
    ...
    println!(
        "Valley (Coupling Gap): {:.3} (n={})",
        v_idx as f64 / n_bins as f64,
        v_val
    );
...
fn compute_conditional_variance(
    ...
    if total_samples == 0 {
        ...
        total_weighted_var / total_samples as f64
    }
}

fn compute_2d_conditional_variance(
    data: &Vec<Vec<f64>>,
    idx_c1: usize,
    idx_c2: usize,
    idx_val: usize,
