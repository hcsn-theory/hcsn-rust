...
fn main() {
    ...
    let total_steps = env::var("HCSN_STEPS").unwrap_or("250000".to_string()).parse::<usize>().unwrap_or(250000);
    let p_create = env::var("HCSN_P_CREATE").unwrap_or("0.58".to_string()).parse::<f64>().unwrap_or(0.58);
    ...
    (0..num_threads).into_par_iter().for_each(|tid| {
        ...
        println!("  Thread {}: Initiating {} step Hyper-Flow simulation...", tid, steps_per_thread);
